...
We treat ratings of ≤ 1 and ≥ 4.5 (on a 0.5–5 star scale) as extreme. Our main statistical question is:
...
ratings.csv – individual ratings
...
```{r}
#| label: extreme-bar
#| fig-cap: "Counts of low, middle, and high ratings."

ggplot(extreme_counts, aes(x = extreme, y = n, fill = extreme)) +
geom_col() +
labs(
title = "Extreme vs Middle Ratings",
x = "Rating Category",
y = "Number of Ratings"
) +
theme_minimal(base_size = 14) +
...
We see the proportions of low, middle, and high ratings, which provides context for our hypothesis tests. **Change how this graph looks**

## Expected vs Actual Distribution (edit this)

Under a “non‑polarized” rating system, we would expect a roughly bell‑shaped distribution centered around 3–4 stars, with relatively few ratings in the tails. To visualise this, we compare our observed ratings to an idealised normal curve with mean 3.5 and standard deviation 1.
...
-   p = true proportion of all ratings that are extreme (≤ 1 or ≥ 4.5).
...
We choose **20%** as a practical cutoff for “moderate polarization”. In a typical unimodal system, we would expect a much smaller share of ratings at the extremes. If more than one in five ratings are extreme, we see that as evidence that users lean toward very strong opinions.
...
-   The p‑value from the one‑sample proportion test with p~0~ = 0.20 is extremely small, so we reject H~0~ ​and conclude that **more than 20% of all ratings are extreme.**
...
Talk about review inflation from user made reviews

# Conclusion

Talk about conclusion from discussion and results
